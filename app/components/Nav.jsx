    ...
    <>
      <header className={`nav${scrolled ? ' scrolled' : ''}${scrolled ? '' : ' on-dark'}`}>
        <div className="container">
          <a href="#top" className="wordmark" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>
            {BRAND.wordmark}
          </a>
      ...
      <div className={`drawer${open ? ' open' : ''}`} role="dialog" aria-modal="true">
        ...
        {user ? (
          ...
          <>
            <button className="btn btn-block" style={{ background: 'transparent', color: 'inherit', border: '1px solid rgba(255,255,255,0.25)', marginBottom: 10 }} onClick={() => { setOpen(false); setLoginOpen(true) }}>Log in</button>
